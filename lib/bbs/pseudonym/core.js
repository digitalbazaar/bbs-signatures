...
// FIXME: implement this and below, then: `proof`, `interface`
export async function CoreProofGenWithPseudonym({
  ...
  */
  ...
  // FIXME: do we need to use `random_scalars.at(-2)` instead like before?
  const pseudonym_init_res = PseudonymProofInit(
    context_id, messages.at(-1), random_scalars.at(-1),
    api_id, ciphersuite);
  const [pseudonym] = pseudonym_init_res;
  // generate challenge
