...
export async function BlindVerify({
  ...
  extra_committed_message_scalars = [],
  secret_prover_blind = 0n,
  api_id, ciphersuite
} = {}) {
  assertInstance(Uint8Array, PK, 'PK');
  ...
  assertArray(committed_messages, 'committed_messages');
  assertType('bigint', secret_prover_blind, 'secret_prover_blind');
  if(extra_committed_message_scalars !== undefined) {
    assertArray(
      extra_committed_message_scalars, 'extra_committed_message_scalars');
  }
  if(api_id === undefined) {
    api_id = createApiId(ciphersuite.ciphersuite_id, BLIND_API_ID);
  }
  ...
  7. blind_generators = BBS.create_generators(
                          length(committed_message_scalars),
                          "BLIND_" || api_id)
  8. res = BBS.CoreVerify(
             PK, signature,
             generators.append(blind_generators),
             header, message_scalars.append(committed_message_scalars),
             api_id)
  9. return res
