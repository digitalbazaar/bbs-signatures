...
export async function calculate_random_scalars({count, ciphersuite} = {}) {
  ...
  */
  ...
  for(let i = 0; i < count; ++i) {
    promises[i] = generateRandomScalar(ciphersuite);
  }
  return Promise.all(promises);
}

// FIXME: move all camel-cased functions to `helpers.js` instead of `util.js`
// to differentiate between spec functions and local helpers
export function concatGenerators(...allGenerators) {
  const generators = allGenerators.flat();
...
export async function generateRandomScalar(ciphersuite) {
...
export function serialize({input_array, ciphersuite} = {}) {
  ...
  7.a.    else if el is a Uint8Array (for "Blind" APIs):
              el_octs = el
  ...
  */
  ...
  for(const el of input_array) {
    let octets;
    if(el instanceof G1.Point) {
      octets = ciphersuite.point_to_octets_E1(el);
    } else if(el instanceof G2.Point) {
